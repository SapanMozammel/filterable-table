    ...
    multiSelect.loadOptions = () => {
      ...
      if (
        ...
        optionElementAll.appendChild(
          newElement("label", { text: config.txtAll })
        );
      ...
      div.refresh = () => {
        ...
        onFilter(selected.map((option) => option.text));
        if (
          selected.length > (multiSelect.attributes["max-items"]?.value ?? 10)
        ) {
          div.appendChild(
            newElement("span", {
    ...
    search.addEventListener("input", () => {
      dropdownList
        ...
        .forEach((div) => {
          ...
          div.style.display = innerText.includes(search.value.toLowerCase())
            ? "flex"
            : "none";
        });
    });
